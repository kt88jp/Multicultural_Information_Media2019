...
public:
	...
	void set(const int x, const int y, const char value) {
		...
		fieldrow[y] = (fieldrow[y] & (0xffffffffffffffff - (0x1fLL << (x * 5)))) | (static_cast<uint64_t>(value) << (x * 5));
	...
	double transition(const int px, const int r, const Pack p, bool debug = false) {
		...
		while (true) {
			...
			if (chain >= 0) {
				...
				for (int y = 0; y < Height; y++) {
					//if (debug) {
					//	std::cerr << "frow " << y << " " << std::hex << fieldrow[y] << "," << (fieldrow[y] >> 5) << std::dec << std::endl;
					//}
					uint64_t rowresult = fieldrow[y] + (fieldrow[y] >> 5);
					//if (debug) {
					//	std::cerr << "row " << std::hex << rowresult << std::dec << std::endl;
					//}

					for (int x = 0; x < Width - 1; x++) {
						if (debug) {
							std::cerr << "rr " << x << " " << (rowresult & 0x1f) << std::endl;
						}
						if ((rowresult & 0x1f) == 10) {
							isDisappeared = true;
							disappear[x][y] = true;
							disappear[x + 1][y] = true;
						}
					...
					if (y < Height - 1) {
						rowresult = fieldrow[y] + fieldrow[y + 1];

						for (int x = 0; x < Width; x++) {
							if ((rowresult & 0x1f) == 10) {
								isDisappeared = true;
								disappear[x][y] = true;
								disappear[x][y + 1] = true;
							}
					...
					if (y < Height - 1) {
						rowresult = fieldrow[y] + (fieldrow[y + 1] >> 5);

						for (int x = 0; x < Width - 1; x++) {
							if ((rowresult & 0x1f) == 10) {
								isDisappeared = true;
								disappear[x][y] = true;
								disappear[x + 1][y + 1] = true;
							}
	...
	uint64_t calc_hash() {
		return hash_64_fnv1a(this, sizeof(BitField2));
	}
};
